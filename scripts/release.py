#!/usr/bin/env python3
"""Zip + publish Google gateway to the UEFN Ducky Store via uds_release.

Usage:
  py scripts/release.py --publish --changelog "v1: Google Gemini + Gemini CLI"
"""
...
def publish(zip_path: Path, *, category: str, changelog: str) -> None:
    ...
    result = mcp_call(
        ...
        "uds_release",
        {
            "zipB64": zip_b64,
            "category": category,
            "changelog": changelog or "v1: Google Gemini API key + Gemini CLI for Settings → LLMs",
            "publish": True,
            "categories": ["plugins", "gateways"],
            "tags": ["google", "gemini", "gemini-cli", "llm", "gateway", "api-key"],
            "name": "Google",
            "description": (
                "Google Gemini API key and Gemini CLI coding agent. After Install + Enable, Google "
                "appears under Settings → LLMs → Providers & Keys, and Gemini CLI under Coding Agents "
